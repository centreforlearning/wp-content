    ...
    while (have_posts()) :
        ...
        foreach ($slugs as $slug) {
            ...
            while ($loop->have_posts()) : $loop->the_post();
                ...
                if (!empty($bgParallax)) $bgclasses[] = 'js-coll-parallax';
                ...
                if (!empty($bgParallax)) {
                    ...
                    switch ($bgtype) {
                        case 'image':
                            ...
                            $dim = wp_get_attachment_image_src(coll_get_attachment_id($bgImg), 'parallax-bg');
                        ...
                        case 'slider':
                            ...
                            if ($bgSliderType == 'flex') {
                                ...
                                $bg_output .= do_shortcode('[coll_flexslider id="' . $bgSliderId . '" class="coll-bg-slider" imgsize="parallax-bg"]');
                                // $bg_output .= '</div>';
                            }
                        ...
                        case 'video':
                            ...
                            if (!$coll_is_mobile) {
                                ...
                                if (!empty($bgRepImg)) {
                                    $image = wp_get_attachment_image_src(coll_get_attachment_id($bgRepImg), 'parallax-bg');
                                    $bg_output .= '<img class="coll-bg-image js-coll-lazy"
					                                    src="data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAAEAAAABAQMAAAAl21bKAAAAA1BMVEUAAACnej3aAAAAAXRSTlMAQObYZgAAAApJREFUCNdjYAAAAAIAAeIhvDMAAAAASUVORK5CYII="
					                                    width="' . $image[1] . '"
					                                    height="' . $image[2] . '"
                ...
                if ($bgOverlay) {
                    ...
                    if (!empty($bgOverlay['background-size'])) {
                        $style .= "background-size: " . $bgOverlay['background-size'] . ";\n";
                    }
                    if ($bgOverlayOpacity) {
				...
				if( is_front_page() && $mobilesection ) {
					$mobilesection = false;
					$output .= '<section id="' . $post->post_name . '-mobile" class="' . implode(array_diff(get_post_class($sclasses, get_the_ID()),['js-coll-window-min']), ' ') . '">';
					$output .=  do_shortcode('[coll_flexslider id="' . $bgSliderId . '" class="coll-bg-slider" imgsize="parallax-bg"]');
					$output .= '<div class="section-content row ' . $showContent . '">';
					$output .= '<div class="columns entry-title ' . $showTitle . '" ' . $titleBorder . '>';
					$output .= '<span class="sep_holder sep_holder_l"><span class="sep_line"></span></span>';
					$output .= '<h2 class="title">' . get_the_title() . '</h2>';
					$output .= '<span class="sep_holder sep_holder_r"><span class="sep_line"></span></span>';
					$output .= '</div>';
                ...
                // build  section
                if (isMobile) $sclasses = array_diff(get_post_class($sclasses, get_the_ID()),['js-coll-window-min']);
