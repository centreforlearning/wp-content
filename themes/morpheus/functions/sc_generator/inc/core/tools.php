...
class Su_Tools
{
    ...
    public static function coll_get_term_slugs($tax = 'category')
    {
        $terms = array();
        foreach ((array)get_terms($tax, array('hide_empty' => false)) as $term) $terms[$term->slug] = $term->name;
        return $terms;
    }
    public static function get_sliders($posttype = 'coll-flexslider')
    {
        $sliders = array();
        $args = array(
            'post_type' => $posttype,
