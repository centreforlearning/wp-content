(function ($) {
    ...
    $(function () {
        ...
        var Page = new function () {
            ...
            this.init = function () {
                ...
                select.change(function (e) {
                    var _this = $(this),
                        ...
                        _ID = _this.find('option:selected').attr('data-ID');

                    createSection(_slug, _name, _ID);
                });
            ...
            var loadItems = function () {
                ...
                if (data) {
                    for (var i = 0; i < data.length; i++) {
                        ...
                        addSection(data[i].name, data[i].slug, data[i].ID)
            ...
            var createSection = function (slug, name, ID) {
                addSection(name, slug, ID);
                // save info
                var _info = {'slug': slug, 'name': name, 'ID': ID};
                data.push(_info)
                updateData();
            }
            var addSection = function (name, slug, ID) {
                var _item = $('' +
                    '<li class="item page-section js-coll-section-item ui-state-default">' +
                    '<a href="post.php?post='+ID+'&action=edit" class="edit"><i class="fa fa-pencil"></i>Edit</a>' +
